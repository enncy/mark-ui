...
export function getRender(renderKey) {
...
export function resolveRaw(render) {
    // 获取 html ， 如果有 pre 子节点，优先获取 pre 的
    let html = render?.querySelector('pre').innerHTML || render?.innerHTML;
    // 格式化 html
 ...
 * @returns
 */
export function scrollToLine(target, change_line) {
    // 根据被改动的行进行高度调整
    ...
    for (const el of els) {
        ...
        if (start <= change_line && change_line <= end && temp_offsetHeight !== el?.offsetTop) {
            ...
            target.scrollTo({
                behavior: "smooth",
                top: el?.offsetTop > target.offsetHeight
 ...
 * @param content
 */
export function formatPreElementContent(content) {
    let res = content;
    let tabInfo = [];
    let min = 0;
    // 收集每行的制表符数量
 ...
 * 颜色修改
 */
export function autoChangeStyle(render) {
    const selects = Array.from(render.querySelectorAll("[" + DATA_SELECTOR_KEY + "]"));
    if (selects.length !== 0) {
...
export function contentCopy(content, render, copyHandler) {
 ...
 */
export function changeStyle(select) {
    // 获取绑定的 selector key
    const key = select.getAttribute(DATA_SELECTOR_KEY);
    const list = document.querySelector(`[${DATA_CODE_KEY}="${key}"]`)?.classList;
