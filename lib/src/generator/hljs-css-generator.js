 ...
 */
export async function hljsLessGenerator({ styles = ["default"], source = "jsDriver", version = defaultVersion, selector }) {
    console.clear();
    ...
    for (const style of styles) {
        const source_resolve = supportSources[source].replace(CDN_VERSION_REPLACEMENT, version);
 ...
 */
export async function hljsCssGenerator(options) {
    const css = await hljsLessGenerator(options);
    return await renderCss(css);
}
export async function renderCss(css) {
 ...
 * @see hljsCssCompress
 */
export function hljsCssCompress(css) {
